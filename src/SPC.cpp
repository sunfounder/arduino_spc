...
uint16_t SPC::readOutputVoltage() {
...
uint16_t SPC::readBatteryVoltage() {
...
int16_t SPC::readBatteryCurrent() {
...
uint8_t SPC::readBatteryPercentage() {
...
uint16_t SPC::readBatteryCapacity() {
...
bool SPC::readIsBatteryPluggedIn() {
  if (!this->device.peripherals.is_plugged_in)  {
...
uint8_t SPC::readShutdownRequest() {
...
uint8_t SPC::readShutdownBatteryPercentage() {
...
uint8_t SPC::readBoardId() { return i2c.readU8(REG_READ_BOARD_ID); }

uint16_t SPC::readPowerSourceVoltage() {
...
void SPC::writeFanPower(uint8_t power) {
  if (!this->device.peripherals.fan_power) {
...
void SPC::writeShutdownBatteryPercentage(uint8_t percentage) {
  if (!this->device.peripherals.shutdown_battery_percentage) {
...
String SPC::readFirmwareVersion() {
